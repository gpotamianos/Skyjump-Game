...
#include <iostream>
//Εδω οριζουμε τι γινεται οταν ο παικτης κανει intersect με τα διαφορα αντικειμενα στο παιχνιδι
...
bool Collisions::checkWinCollision(const Box& spaceship)
{
    
    if (m_state->getPlayer()->intersect(const_cast<Box&>(spaceship)))
    {
...
bool Collisions::checkEnemyCollision(const std::vector<Box>& objects)
{
    if (!objects.empty()) {
        for (auto& block : objects)
        {
            if (m_state->getPlayer()->intersect(const_cast<Box&>(block)))
            {
                return true;
            }
        }
